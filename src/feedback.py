...
class FeedbackStore:
    ...
    def _session(self, session_id: str) -> dict[str, set]:
        return self.sessions.setdefault(session_id, {"liked": set(), "disliked": set()})

    def record(self, session_id: str, product_id: str, liked: bool):
    ...
    def apply_rocchio(self, session_id: str, query_vector: np.ndarray, store: FaissVectorStore,
                       alpha: float = 1.0, beta: float = 0.75, gamma: float = 0.15) -> np.ndarray:
        session = self.sessions.get(session_id)
        if not session or not (session["liked"] or session["disliked"]):
            return query_vector

        liked_vecs = [store.reconstruct_by_product_id(pid) for pid in session["liked"]]
        liked_vecs = [v for v in liked_vecs if v is not None]
        disliked_vecs = [store.reconstruct_by_product_id(pid) for pid in session["disliked"]]

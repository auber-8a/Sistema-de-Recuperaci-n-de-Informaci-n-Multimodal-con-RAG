...
def _como_tensor(output: torch.Tensor) -> torch.Tensor:
    """
...
class ClipEmbedder:
    ...
    @torch.no_grad()
    def codificar_textos(self, texts: list[str], batch_size: int = 32) -> np.ndarray:
        all_embeds = []
        for i in range(0, len(texts), batch_size):
            ...
            feats = _como_tensor(self.model.get_text_features(**inputs))
            feats = feats / feats.norm(p=2, dim=-1, keepdim=True)
            all_embeds.append(feats.cpu().numpy())
        return np.concatenate(all_embeds, axis=0).astype("float32")

    @torch.no_grad()
    def codificar_imagenes(self, image_paths: list[str], batch_size: int = 16) -> np.ndarray:
        all_embeds = []
        for i in range(0, len(image_paths), batch_size):
            batch_paths = image_paths[i:i + batch_size]
            images = [Image.open(p).convert("RGB") for p in batch_paths]
            inputs = self.processor(images=images, return_tensors="pt").to(self.device)
            feats = _como_tensor(self.model.get_image_features(**inputs))
            feats = feats / feats.norm(p=2, dim=-1, keepdim=True)
            all_embeds.append(feats.cpu().numpy())
        return np.concatenate(all_embeds, axis=0).astype("float32")

    def codificar_consulta(self, query: str) -> np.ndarray:
        """Embedding de una consulta de texto (misma proyeccion que codificar_textos)."""
        return self.codificar_textos([query])[0]

    def codificar_documento_multimodal(self, text: str, image_path: str | None,
        ...
        """
        text_emb = self.codificar_textos([text])[0]
        if image_path is None:
            return text_emb
        image_emb = self.codificar_imagenes([image_path])[0]
...
if __name__ == "__main__":
    ...
    v = embedder.codificar_consulta("wireless bluetooth headphones")
